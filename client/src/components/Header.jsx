...
//If user is not logged in, clicking on Generate Images button will show login page 
const Header = () => {
        ...
        <motion.div className='flex flex-col justify-center items-center text-center my-20' 
            ...
            className='flex flex-wrap justify-center mt-16 gap-3'>
                {Array(6).fill('').map((item, index) => (                                 // map function to create an array of 6 items and show 6 images
                    ...
                    className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' src={index%2 == 0 ? assets.sample_img_1 : assets.sample_img_2 } alt='' key={index} width={70}></motion.img> // On alternate indices, show different images. by checking if index is even or odd by divisibility by 2
                    ))
                }
            </motion.div>
                ...
                className='mt-2 text-neutral-600'>Generated images from MindSketch</motion.p>
...
//Note:
// To use Framer Motion, infront of tag only add "motion." to use it ; after that add functionalities like initial, animate, transition, whileInView, etc.
// Eg., <motion.div> instead of <div>